    ...
    return (
        <>
            ...
            <div className='popup scale-in-top'>
                ...
                {ifAmbulance && <span style={{ fontSize: "1.2vw" }}>You will be redirected to ambulance form.</span>}
                <div className='popupbtn'>
                    <button className='cancelbtn' onClick={() => { setPopUp(false) }}>Cancel</button>
                    <button className='nextbtn' onClick={handleSubmit}><Link
                        style={{ color: "white", textDecoration: "none" }}
                        to={ifAmbulance ? "/facilityinformation" : "/"}>Submit</Link></button>
                </div>
            </div>
        </>
    )
